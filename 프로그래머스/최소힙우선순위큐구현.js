class MinHeap {
  ...
  pop() {
    ...
    const min = this.items[0]; // 루트 노드(가장 작은 값) 저장
  ...
  bubbleUp() {
    ...
    while (index > 0) {
      ...
      if (this.items[parentIndex] <= this.items[index]) {
        break; // 부모 노드가 현재 노드보다 작거나 같으면 반복 종료
      }
    ...
    while (index * 2 + 1 < this.size()) {
      ...
      let smallerChild =
        rightChild < this.size() && this.items[rightChild] < this.items[leftChild]
          ? rightChild // 오른쪽 자식이 더 작으면 오른쪽 선택
          : leftChild; // 그렇지 않으면 왼쪽 선택

      if (this.items[index] <= this.items[smallerChild]) {
        break; // 현재 노드가 자식보다 작거나 같으면 반복 종료
      }

      this.swap(index, smallerChild); // 현재 노드와 더 작은 자식의 위치를 교환
      index = smallerChild; // 현재 노드의 위치를 자식으로 업데이트
    }
  }
}
